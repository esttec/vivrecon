...
class AuthService(
    ...
    @Transactional
    fun register(req: RegisterRequest, deviceInfo: String): AuthResponse {
        require(!userRepo.existsByEmail(req.email)) { "Email already in use" }
        val user = userRepo.save(
            UserEntity(email = req.email.lowercase(), passwordHash = passwordEncoder.encode(req.password))
        )
